...
2. **구현이 아닌 행동을 검증한다**: 구현 의존 테스트는 리팩토링 시 함께 깨진다. 행동 중심 테스트는 내부 변경에 강하다.
   - 나쁜 예: `optionRepository.save()`가 호출되었는지 검증
   - 좋은 예: 재고 1개 상품에 선물 요청 2번 → 1번 성공, 2번 실패
3. **테스트 유형은 상황에 맞게 판단**: 무조건 단위 테스트를 선택하지 않는다. 행위의 특성에 따라 적절한 테스트 유형을 제안한다.
   - 도메인 로직만 검증 → Unit
...
해당하지 않으면 하위 레벨(통합/단위)로 분배한다.

## 시나리오 도출 흐름 (outside-in)

1. **사용자 행동 정의** → 기능 파악
2. **성공/실패 기준 정의** → 인수 조건 도출
3. **관찰 가능한 결과 정의** → 시나리오 도출
4. **필요한 하위 검증은 다른 레벨에 분배**
