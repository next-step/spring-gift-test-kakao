...
변경사항이 여러 의도를 포함하면 커밋을 나눈다.

### 판단 질문

> "이 커밋의 diff를 보는 리뷰어가, 모든 변경 줄에 대해 **같은 이유**를 떠올릴 수 있는가?"
...
| 신호 | 예시 |
|------|------|
| 커밋 메시지에 "그리고"가 필요함 | `@Transactional 제거 그리고 메서드명 변경` |
| type이 다름 | `refactor` + `test`가 섞여 있음 |
| scope가 다름 | `category` 변경 + `gift` 변경이 섞여 있음 |

### 합쳐야 하는 경우

| 신호 | 예시 |
|------|------|
| 같은 이유에서 나온 변경 | 메서드명 변경 + 그로 인해 불필요해진 주석 제거 |
| 나누면 중간 커밋에서 테스트가 깨짐 | 인터페이스 변경 + 구현체 변경 |

### 실행 규칙

- 변경사항 분석 시 의도가 2개 이상이면 의도별로 커밋을 나누어 실행한다
