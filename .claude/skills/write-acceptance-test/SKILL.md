...
- **테스트가 검증하는 행위를 정확히 하나로 식별한다** — "생성"과 "조회"는 별개의 행위이므로 하나의 테스트에서 섞지 않는다
  - 조회 테스트: SQL로 데이터를 세팅하고, GET 요청으로 결과를 검증한다
  - 생성 테스트: API(POST)를 호출하여 생성하고, 응답 또는 후속 조회로 검증한다
  - @DisplayName은 테스트가 실제로 수행하는 행위만 서술한다 (예: SQL 삽입 후 GET만 하면 "조회한다"이지 "생성하면 조회된다"가 아니다)
