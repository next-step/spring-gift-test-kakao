...
프로젝트 초기에 작업 프롬프트 형태의 `Claude.md`가 있었는데, Claude Code 공식 best practice와 맞지 않았다. best practice에 따르면 CLAUDE.md는 일회성 작업 지시가 아니라 프로젝트 규칙과 컨벤션을 간결하게 담는 파일이어야 한다.

AI에게 공식 문서 기준으로 검토를 요청했고, 다음 문제를 발견하여 수정했다.

- 파일명 `Claude.md` → `CLAUDE.md` (공식 네이밍 컨벤션)
