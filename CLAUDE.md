...
- `build.gradle`의 `testImplementation`/`testRuntimeOnly` 의존성 추가 및 커스텀 Gradle task 정의는 테스트 인프라이므로 허용한다.
- 프로젝트 루트의 인프라 파일 (`Dockerfile`, `docker-compose.yml`, `.dockerignore`) 생성/수정은 허용한다.
