...
class ProductAcceptanceTest {
    ...
    void 상품_생성_성공() {
        // Given: 카테고리(id=1, 전자기기)가 존재하는 상태
        var request = Map.of(
                "name", "맥북 에어",
                "price", 1500000,
        ...
        // When: 상품 생성 API 호출
        // Then: 200 OK, id가 자동 생성되고 요청 값이 그대로 반환된다
        RestAssured
                .given().log().all()
                ...
                .then().log().all()
                    ...
                    .body("imageUrl", equalTo("https://example.com/macbook.png"))
    ...
    void 상품_생성_후_목록에서_조회된다() {
        // Given: 카테고리(id=1)만 존재하고 상품은 없는 상태
        var request = Map.of(
                "name", "맥북 에어",
                "price", 1500000,
                "imageUrl", "https://example.com/macbook.png",
    ...
    void 상품_여러건_조회_성공() {
        // Given: 상품 3건이 존재하는 상태 (seed.sql 기반)

        // When: 상품 목록 조회 API 호출
        // Then: 200 OK, 3건 모두 반환되며 이름과 가격이 정확하다
        RestAssured
                .given().log().all()
                .when()
                    .get("/api/products")
                .then().log().all()
                    ...
                    .body("name", hasItems("맥북 에어", "아이패드", "패딩 점퍼"))
                    .body("price", hasItems(1500000, 800000, 250000));
    }
    ...
    void 존재하지_않는_카테고리로_상품_생성_실패() {
        ...
        // When: 존재하지 않는 카테고리(id=999)로 상품 생성 시도
        // Then: 500 에러 (NoSuchElementException - categoryRepository.findById().orElseThrow())
        RestAssured
                .given().log().all()
                    .contentType(ContentType.JSON)
                    .body(request)
